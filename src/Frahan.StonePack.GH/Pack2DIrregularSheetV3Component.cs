...
/// <summary>
/// Version 3 — handles any closed planar curve (freeform, arc, spline, polyline).
/// Converts all inputs to polyline abstractions at construction time; hot path is pure math.
/// Fixes V2 candidate generation for non-convex and organic sheet outlines.
/// </summary>
...
public sealed class Pack2DIrregularSheetV3Component : GH_TaskCapableComponent<PackingResult>
{
    public Pack2DIrregularSheetV3Component()
        : base("2D Freeform Sheet Pack V3", "Freeform V3",
            ...
            "Pack any closed planar curves into freeform sheet outlines. " +
            "Converts all inputs to polyline polygons for robust containment on organic shapes. " +
            "Non-blocking async solve. Implements NFP-assisted bottom-left nesting (Burke et al. 2007).",
            "Frahan", "2D Packing")
    {
    }
    ...
    // (Variant=3). Will be removed in 0.8.0 per R3 plan PR 9. Existing GH
    ...
    public override Guid ComponentGuid => new Guid("C9D74E3F-5A06-4D2B-BEF3-A7F96C4E2D5A");
    ...
    protected override void SolveInstance(IGH_DataAccess da)
    {
        if (InPreSolve)
        {
            ...
            {
                var solver = new IrregularSheetFillV3(
        ...
        if (!GetSolveResults(da, out result))
        {
            ...
            {
                AddRuntimeMessage(GH_RuntimeMessageLevel.Error, "V3 packing failed: " + ex.Message);
                return;
            }
        }
