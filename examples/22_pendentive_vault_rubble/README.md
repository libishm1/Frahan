...
for stable mortarless assembly. Each cell is an intrados patch extruded radially by the thickness. The
cells are generated by the **Pendentive Vault Voussoirs** component (Frahan > Voussoir, GUID D5F10013).
...
- Pendentive dome, R = 2.5 m, span 3.2 m, 36 voussoirs, shell volume 5.6941 m^3.
- **36/36 real rubble trims**, 0 clean fallback; **coverage 98.3%** (recovered / shell volume).
- Cells from the Pendentive Vault Voussoirs component; each matched to the best of 6 ETH1100 candidate
  stones (scaled to 1.6x the cell diagonal, centroid-aligned) and CGAL-intersected to the cell.
- **Fix (the reported bug):** every voussoir is now either a CGAL-trimmed rubble stone bounded by its
  cell, or (if no valid trim) the CLEAN cut cell. The earlier version kept raw un-trimmed boulders on
  failed trims, which overshot the vault. No raw boulders remain. Metrics in `22_vault_metrics.json`.
