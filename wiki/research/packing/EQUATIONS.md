...
Oliveira 2009 (JORS tutorial); Stoyan et al. 2002. Sanity check: two unit
squares give $\mathrm{NFP}=[-1,1]^2$, area $4$.
...
guaranteed. Sanity check: $[0,5]^2\ominus$ unit square $=[0,4]^2$, area $16$.
...
### 2.1 Voxel feasibility (true-3D collision-free poses)
...
### 2.2 Deepest-Bottom-Left constructive placement
...
### 2.3 Void-fill relaxation (constructive settle)
...
### 2.4 Convex-decomposition collision proxy
...
### 2.5 Physics settle (rigid-body dynamics to static equilibrium)
...
Settling is integration until total kinetic energy
$\sum_i(\tfrac12 m_i\lVert v_i\rVert^2+\tfrac12\omega_i^{\top}I_i\omega_i)\to 0$
(bodies sleep) — static equilibrium. Production code: `BulletSettleService`
(Bullet sequential-impulse solver, CoACD compound shapes). Citation: Zhuang
et al. 2024 (Computers & Graphics 123:103996); Bullet; Coulomb friction.

### 2.6 Stability (the result must stand)
...
### 2.8 Complexity (from the code)

Constructive: one FFT correlation per (stone, orientation) over the grid,
$O(N\,k\,G\log G)$ for $N$ stones, $k$ orientations, grid size $G$. Settle:
rigid-body steps dominated by the contact solver over the convex pieces,
near-linear per step in active contacts.
